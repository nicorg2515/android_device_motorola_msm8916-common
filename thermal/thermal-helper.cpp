...
ssize_t fillTemperatures(hidl_vec<Temperature> *temperatures) {
    ...
    if (current_index < temperatures->size()) {
        // CPU_therm: temperature in Celsius.
        result = readTemperature(kTsensOffset + kCpuTsensOffset[0], TemperatureType::CPU, kCpuLabel[0],
    ...
    // GPU temperature.
    if (current_index < temperatures->size()) {
        // GPU_therm: temperature in Celsius.
        result = readTemperature(kTsensOffset + kGpuTsensOffset, TemperatureType::GPU, kGpuLabel,
